...
//Added code from PARTICLE SYSTEM (base-code.sln)

// All objects on scene to be rendered 
...
*/
std::vector<GameObject*> populateScene() {
	std::vector<GameObject*> compiled;

	compiled.push_back(new BurnGrid(glm::vec3(0), particles));
...
//END of imported functions/variables from Particle System
...
void idle()
{
	updateObjects(); //Updating Particles
	glutPostRedisplay();
}
...
void display()
{
	...
	//ADDED CODE from Particle System
	calculateDeltaTime();
...
void main(int argc, char** argv)
{
	...
	glutIdleFunc(updateObjects); //Updating Particles
	timer(0);
	glewInit();
	init();
	glutMainLoop();
